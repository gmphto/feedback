...
test('request logs retain route/status without query, headers, raw paths or provider errors', async t => {
  ...
  t.after(() => app.close());
  app.get('/auth/provider-failure', async () => { throw new Error('private-provider-payload'); });
  for (const url of ['/api/ready?code=private-code&state=private-state', '/auth/provider-failure?error_description=private-description', '/private-path?token=private-token']) {
  ...
  assert.deepEqual(logs.map(({ route, statusCode }) => ({ route, statusCode })), [
    ...
    { route: '/auth/provider-failure', statusCode: 500 },
